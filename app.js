...
app.command('/survey-bjm', async ({ command, ack, payload, context, respond }) => {
...
app.command('/weather-bjm', async ({ command, context, ack }) => {
  ack()
  app.client.chat.postEphemeral({
    token: context.botToken,
...
app.shortcut('save4later-bjm', async ({ shortcut, ack, say, respond, context }) => {
  ack()
